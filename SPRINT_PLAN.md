...
## Pre-Sprint: 생성 앱 스택 확정

> ⚠️ S1 착수 전 반드시 완료

| 작업 | 산출물 |
|------|--------|
| 생성 앱 고정 스택 확정 | PRD에 명시 (예: FastAPI + Next.js + PostgreSQL) |
| 에이전트 skills 템플릿 방향 결정 | 스택에 맞는 skills 가이드 초안 |

> 에이전트 skills 파일(S2)이 스택에 의존하므로 선행 확정 필수.
...
| - 비용 추적 | S3 | BE: 토큰 파싱 + token_usage + GET /cost |
