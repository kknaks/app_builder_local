...
│          │                          │                  │
│   목록    │        대시보드           │      채팅        │
│  (좌측)   │    (n8n 스타일 플로우)    │  (Claude Code)   │
│          │                          │                  │
│ ─────── │                          │                  │
│ 1.아이디어│  [아이디어]               │ > 쇼핑몰 만들어줘 │
│ 2.기획서  │     ↓                    │                  │
│  ├구체화  │  [기획서]─→[BE검토]      │ PM: 기획서 검토   │
│  ├검증   │     │   ─→[FE검토]      │ 결과를 전달합니다  │
│ 3.구현   │     │   ─→[Design검토]  │ ...              │
│  ├백엔드  │     ↓                    │                  │
│  ├프론트  │  [스프린트]               │                  │
...
### 4.3 우측: 채팅 패널 (Claude Code 실시간)

로컬에 연결된 Claude Code와의 실시간 채팅창.
- 현재 활성 단계의 에이전트와 대화
- PM이 유저에게 보고/질문 → 이 채팅으로 전달
- 유저가 승인/피드백 입력 → PM에게 전달
- 에이전트 실행 로그도 여기서 실시간 스트리밍
